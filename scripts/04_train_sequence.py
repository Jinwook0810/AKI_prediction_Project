...
def main() -> None:
    ...
    if args.tune_hyperparameters:
        ...
        for hidden_size, dropout, learning_rate, num_layers in grid:
            set_seed(RANDOM_SEED)
            trial_metrics = train_and_evaluate_lstm(
                x_train,
                y_train,
                x_valid,
                y_valid,
        ...
        if best_metrics is None:
            raise RuntimeError("No LSTM tuning trial completed.")
        ...
        metrics["tuning"] = {
            ...
            "n_trials": len(trials),
            "best_params": {
                "hidden_size": metrics["training"]["hidden_size"],
    ...
    if args.tune_hyperparameters:
        if args.use_mask:
            metrics_filename = "lstm_sequence_masked_tuned_v1.json"
        else:
            metrics_filename = "lstm_sequence_tuned_v1.json"
    else:
        metrics_filename = "lstm_sequence_masked_v1.json" if args.use_mask else "lstm_sequence_v1.json"
